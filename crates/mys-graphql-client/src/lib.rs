...
use mys_types::framework::Coin;
use mys_types::Address;
use mys_types::CheckpointDigest;
use mys_types::CheckpointSequenceNumber;
use mys_types::CheckpointSummary;
use mys_types::Event;
use mys_types::MovePackage;
use mys_types::Object;
use mys_types::SignedTransaction;
use mys_types::Transaction;
use mys_types::TransactionDigest;
use mys_types::TransactionEffects;
use mys_types::TransactionKind;
...
const MAINNET_HOST: &str = "https://mys-mainnet.mystenlabs.com/graphql";
const TESTNET_HOST: &str = "https://mys-testnet.mystenlabs.com/graphql";
const DEVNET_HOST: &str = "https://mys-devnet.mystenlabs.com/graphql";
...
/// The GraphQL client for interacting with the MySocial blockchain.
...
impl Client {
    ...
    /// Coin type will default to `0x2::coin::Coin<0x2::mys::MYS>` if not provided.
    ...
    /// coins. For MYS coin, pass in the coin type: `0x2::coin::Coin<0x2::mys::MYS>`.
    ...
    /// ```rust,ignore
    ///
    /// let client = mys_graphql_client::Client::new_devnet();
    ...
    pub async fn object(&self, address: Address, version: Option<u64>) -> Result<Option<Object>> {
        ...
        if let Some(object) = response.data {
            ...
            let object = bcs
                .map(|b| bcs::from_bytes::<mys_types::Object>(&b))
                .transpose()?;
...
mod tests {
    ...
    async fn test_coin_metadata_query() {
        let client = test_client();
        let cm = client.coin_metadata("0x2::mys::MYS").await;
        assert!(
            cm.is_ok(),
    ...
    async fn test_total_supply() {
        let client = test_client();
        let ts = client.total_supply("0x2::mys::MYS").await;
        assert!(
            ts.is_ok(),
    ...
    async fn test_package_by_name() {
        let client = Client::new_testnet();
        let package = client.package_by_name("mys@mys").await;

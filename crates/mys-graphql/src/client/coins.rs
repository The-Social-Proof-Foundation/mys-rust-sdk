...
use futures::Stream;
use mys_graphql_macros::Response;
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use mys_graphql::Client;
    /// use mys_sdk_types::Address;
    /// use mys_sdk_types::StructTag;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ///     let client = Client::new("https://mys-mainnet.mystenlabs.com/graphql")?;
    ///     let owner: Address = "0x123...".parse()?;
    ///
    ///     // Get MYS balance using the helper
    ///     let mys_balance = client.get_balance(owner, &StructTag::mys()).await?;
    ///     if let Some(bal) = mys_balance {
    ///         println!("MYS balance: {}", bal.total_balance);
    ///     }
    ///
...
mod tests {
    ...
    async fn test_get_balance_found() {
        ...
        let result = client.get_balance(owner, &StructTag::mys()).await;
        assert!(result.is_ok());
        ...
        assert_eq!(balance.coin_type, StructTag::mys());
    ...
    async fn test_list_balances_multiple() {
        ...
        assert_eq!(bal1.coin_type, StructTag::mys());

...
use crate::proto::mys::rpc::v2::Argument;
use crate::proto::mys::rpc::v2::GetObjectRequest;
use crate::proto::mys::rpc::v2::Input;
use crate::proto::mys::rpc::v2::ListOwnedObjectsRequest;
use crate::proto::mys::rpc::v2::MoveCall;
use crate::proto::mys::rpc::v2::Object;
use crate::proto::mys::rpc::v2::ProgrammableTransaction;
use crate::proto::mys::rpc::v2::SimulateTransactionRequest;
use crate::proto::mys::rpc::v2::Transaction;
use crate::proto::mys::rpc::v2::simulate_transaction_request::TransactionChecks;
...
impl Client {
    pub async fn get_delegated_stake(&mut self, staked_mys_id: &Address) -> Result<DelegatedStake> {
        let maybe_staked_sui = self
            .ledger_client()
            .get_object(
                GetObjectRequest::new(staked_mys_id)
    ...
    async fn calculate_rewards(
        ...
        if staked_mys_ids.len() != resp.command_outputs.len() {
            return Err(tonic::Status::from_error(
                "missing transaction command_outputs".into(),
            ));
        }

        let mut rewards = Vec::with_capacity(staked_mys_ids.len());

        for (id, output) in staked_mys_ids.iter().zip(resp.command_outputs) {

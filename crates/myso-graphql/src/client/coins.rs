...
use futures::Stream;
use myso_graphql_macros::Response;
...
#[derive(Debug, Clone)]
pub struct Balance {
    /// The coin type (e.g., `0x2::myso::MYSO`).
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use myso_graphql::Client;
    /// use myso_sdk_types::Address;
    /// use myso_sdk_types::StructTag;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ///     let client = Client::new("https://myso-mainnet.mystenlabs.com/graphql")?;
    ///     let owner: Address = "0x123...".parse()?;
    ///
    ///     // Get MYSO balance using the helper
    ///     let myso_balance = client.get_balance(owner, &StructTag::myso()).await?;
    ///     if let Some(bal) = myso_balance {
    ///         println!("MYSO balance: {}", bal.total_balance);
    ///     }
    ///
...
mod tests {
    ...
    async fn test_get_balance_found() {
        ...
        let result = client.get_balance(owner, &StructTag::myso()).await;
        assert!(result.is_ok());
        ...
        assert_eq!(balance.coin_type, StructTag::myso());
    ...
    async fn test_list_balances_multiple() {
        ...
        assert_eq!(bal1.coin_type, StructTag::myso());

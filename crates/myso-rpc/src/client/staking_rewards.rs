...
use crate::proto::myso::rpc::v2::Argument;
use crate::proto::myso::rpc::v2::GetObjectRequest;
use crate::proto::myso::rpc::v2::Input;
use crate::proto::myso::rpc::v2::ListOwnedObjectsRequest;
use crate::proto::myso::rpc::v2::MoveCall;
use crate::proto::myso::rpc::v2::Object;
use crate::proto::myso::rpc::v2::ProgrammableTransaction;
use crate::proto::myso::rpc::v2::SimulateTransactionRequest;
use crate::proto::myso::rpc::v2::Transaction;
use crate::proto::myso::rpc::v2::simulate_transaction_request::TransactionChecks;
...
impl Client {
    pub async fn get_delegated_stake(&mut self, staked_myso_id: &Address) -> Result<DelegatedStake> {
        let maybe_staked_myso = self
            .ledger_client()
            .get_object(
                GetObjectRequest::new(staked_myso_id)
        ...
        let mut stakes = self
            .try_create_delegated_stake_info(&[maybe_staked_myso])
            .await?;
        Ok(stakes.remove(0))
    }
    ...
    async fn try_create_delegated_stake_info(
        &mut self,
        maybe_staked_myso: &[Object],
    ) -> Result<Vec<DelegatedStake>> {
        let staked_mysos = maybe_staked_myso
            .iter()
            .map(|o| {
                o.contents()
        ...
        Ok(staked_mysos
            ...
            .map(
                |((staked_myso, (_id, rewards)), (_pool_id, validator_address))| DelegatedStake {
                    staked_myso_id: staked_myso.id,
                    validator_address,
                    staking_pool: staked_myso.pool_id,
                    activation_epoch: staked_myso.stake_activation_epoch,
                    principal: staked_myso.principal,
                    rewards,
                },
            )
            .collect())
    ...
    async fn calculate_rewards(
        ...
        for id in staked_myso_ids {
            let staked_myso = Argument::new_input(ptb.inputs.len() as u16);
            ...
            ptb.commands.push(
                MoveCall::default()
                    .with_package("0x3")
                    .with_module("myso_system")
                    .with_function("calculate_rewards")
                    .with_arguments(vec![system_object, staked_myso])
                    .into(),
            );
        }
        ...
        if staked_myso_ids.len() != resp.command_outputs.len() {
            return Err(tonic::Status::from_error(
                "missing transaction command_outputs".into(),
            ));
        }

        let mut rewards = Vec::with_capacity(staked_myso_ids.len());

        for (id, output) in staked_myso_ids.iter().zip(resp.command_outputs) {

## Overview
This package defines a number of APIs for interacting with services in the Mys
...
- `x-mys-chain-id`: Chain Id of the current chain
- `x-mys-chain`: Human-readable name of the current chain
- `x-mys-checkpoint-height`: Current checkpoint height
- `x-mys-lowest-available-checkpoint`: Lowest available checkpoint for which
...
- `x-mys-lowest-available-checkpoint-objects`: Lowest available checkpoint for
...
- `x-mys-epoch`: Current epoch of the chain.
- `x-mys-timestamp-ms`: Current timestamp of the chain, represented as number
  of milliseconds from the Unix epoch.
- `x-mys-timestamp`: Current timestamp of the chain, encoded in the
